...
export async function GET(
  ...
  try {
    ...
    const dbRecords: DateableSQL[] = await dataFrom(
        supabaseClient.from('frame_images')
            .select()
            .eq('plan_id', plan_id)
            .eq('user_id', user.id)
    );
    ...
    console.log('>>>>>> --------- error getting frame_images', records, err);
...
export async function POST(req) {
  ...
  try {
    ...
    let saved_on_db = !local_records.length ? [] : await dataFrom(
        supabaseClient.from('frame_images').upsert(
    ...
    console.log('>>>>>> --------- error posting frame_images', records, err);
...
export async function PATCH(req) {
  ...
  try {
    ...
    console.log('>>>>>> --------- error patching frame_images', records, err);
    return new Response(JSON.stringify({error: err.message}), {
      status: 500,
    });
  }
