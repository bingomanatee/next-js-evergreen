...
export async function GET(
  ...
  try {
    ...
    const dbRecords: DateableSQL[] = await dataFrom(
        supabaseClient.from('map_points')
            .select()
            .eq('plan_id', plan_id)
            .eq('user_id', user.id)
    );
    ...
    console.log('>>>>>> --------- error getting map_points', records, err);
...
export async function POST(req) {
  ...
  try {
    ...
    let saved_on_db = !local_records.length ? [] : await dataFrom(
        supabaseClient.from('map_points').upsert(
    ...
    console.log('>>>>>> --------- error posting map_points', records, err);
...
export async function PATCH(req) {
  ...
  try {
    ...
    console.log('>>>>>> --------- error patching map_points', records, err);
    return new Response(JSON.stringify({error: err.message}), {
      status: 500,
    });
  }
